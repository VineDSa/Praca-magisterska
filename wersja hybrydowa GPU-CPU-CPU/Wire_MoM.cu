	...
	int* ipv, clDoubleComplex* cu, clDoubleComplex* ci,  int rank)
{
	...
	// zmatrix
	zmatrix_gpu_and_cpu(cz, x, y, z, rad, ipc, ips, ise, nsmax, ka, npls, ienv, sgnx, sgny, sgnz, sgnenv, rank);
	...
	int ienv, float sgnx, float sgny, float sgnz, float sgnenv, int rank)
{
	...
	cudaMallocAsync((void**)&x_d, nsmax * sizeof(float), stream_1);
	cudaMallocAsync((void**)&y_d, nsmax * sizeof(float), stream_2);
	cudaMallocAsync((void**)&z_d, nsmax * sizeof(float), stream_3);
	cudaMallocAsync((void**)&rad_d, nsmax * sizeof(float), stream_4);
	cudaMallocAsync((void**)&ipc_d, nsmax * sizeof(int), stream_5);
	cudaMallocAsync((void**)&ips_d, 2 * nsmax * sizeof(int), stream_6);
	cudaMallocAsync((void**)&ise_d, 2 * nsmax * sizeof(int), stream_7);
	CUBLAS_CALL(cudaMallocAsync((void**)&cz_d, npls * npls * sizeof(cuDoubleComplex), stream_8), "Zmatrix allloc fail!\n");
	...
	cudaMemcpyAsync(x_d, x_h, nsmax * sizeof(float), cudaMemcpyHostToDevice, stream_1);
	cudaMemcpyAsync(y_d, y_h, nsmax * sizeof(float), cudaMemcpyHostToDevice, stream_2);
	cudaMemcpyAsync(z_d, z_h, nsmax * sizeof(float), cudaMemcpyHostToDevice, stream_3);
	cudaMemcpyAsync(rad_d, rad_h, nsmax * sizeof(float), cudaMemcpyHostToDevice, stream_4);
	cudaMemcpyAsync(ipc_d, ipc_h, nsmax * sizeof(int), cudaMemcpyHostToDevice, stream_5);
	cudaMemcpyAsync(ips_d, ips_h, 2 * nsmax * sizeof(int), cudaMemcpyHostToDevice, stream_6);
	cudaMemcpyAsync(ise_d, ise_h, 2 * nsmax * sizeof(int), cudaMemcpyHostToDevice, stream_7);
	...
	std::cout << std::endl <<" Process - " << rank << " zmatrix mix GPU and CPU execution time = " << std::setprecision(4) << elapsedTime_kernel / 1.0e3 << " s" << std::endl;

	// Copy impedance matrix from GPU to CPU
	cudaMemcpyAsync(cz_h, cz_d, sizeof(cuDoubleComplex) * npls * npls, cudaMemcpyDeviceToHost, stream_1);
	...
	std::cout << std::endl << " Process - " << rank << " zmatrix mix GPU and CPU execution time including data transfer = " << std::setprecision(4) << elapsedTime / 1.0e3 << " s" << std::endl;
	std::cout << std::endl << " Process - " << rank << " zmatrix mix GPU and CPU Data transfer time = " << std::setprecision(4) << (elapsedTime - elapsedTime_kernel) / 1.0e3 << " s" << std::endl;
...

	{
		Timer timer;
		timer.rank = rank;
		timer.info = "fill CPU part";
		cpu_cknn_cwire1_re(cz_h, x_h, y_h, z_h, rad_h, ipc_h, ips_h, ise_h, nsmax, npls, ka, eta0, npls, ii, ij);
		cpu_cknn_cwire2_re(cz_h, x_h, y_h, z_h, rad_h, ipc_h, ips_h, ise_h, nsmax, npls, ka, eta0, npls, ii, ij);

		if (ienv == 2) {
			cpu_cknn_cwire1_im(cz_h, x_h, y_h, z_h, rad_h, ipc_h, ips_h, ise_h, nsmax, npls, ka, eta0, npls, ii, ij);
			cpu_cknn_cwire2_im(cz_h, x_h, y_h, z_h, rad_h, ipc_h, ips_h, ise_h, nsmax, npls, ka, eta0, npls, ii, ij);
		}
	}
